...
def _sanitize_mapping_tree_dn(sanitizer, dn: str) -> str:
    """Sanitize the suffix-bearing RDN of a mapping-tree DN.

    Mapping-tree entries embed user suffixes in their DN, e.g.
    ``cn="dc=acme,dc=com",cn=mapping tree,cn=config``.  These end in
    ``,cn=config`` so :func:`_is_config_dn` would otherwise keep them in
    clear text.  Replace the suffix-bearing RDN value while preserving the
    ``cn=mapping tree,cn=config`` tail (and any leaf RDNs such as
    ``cn=replica``) so the entry stays identifiable.
    """
    try:
        import ldap.dn as _ldn
        parsed = _ldn.str2dn(dn)
        # Tail is [cn=mapping tree, cn=config]; the suffix-bearing RDN
        # sits immediately above it.
        suffix_idx = len(parsed) - 3
        if suffix_idx < 0:
            return dn
        new_parsed = []
        for i, rdn in enumerate(parsed):
            if i == suffix_idx:
                rdn = [(attr, sanitizer.sanitize_suffix(val), 1) for attr, val, _ in rdn]
            new_parsed.append(rdn)
        return _ldn.dn2str(new_parsed)
    except Exception:
        return sanitizer.sanitize_dn(dn)


def _sanitize_config_entry_dn(sanitizer, dn: str) -> str:
    """Sanitize a single DN from a dse.ldif comparison.

    - Mapping-tree DNs carry user suffixes in an RDN value — sanitize
      just that RDN, keeping the ``cn=mapping tree,cn=config`` tail.
    - Other cn=config DNs are server metadata and stay in clear text.
    - Anything else is fully anonymized.
    """
    if is_under_dn(dn, "cn=mapping tree,cn=config"):
        return _sanitize_mapping_tree_dn(sanitizer, dn)
    if _is_config_dn(dn):
        return dn
    return sanitizer.sanitize_dn(dn)
...
def _sanitize_dse_comparison(sanitizer, sanitized: Dict[str, Any]) -> None:
    ...
    names, etc. — not user data.  Mapping-tree DNs are the exception: their
    RDN values embed user suffixes, so those values are sanitized.
    ...
    for list_key in ("only_in_server1", "only_in_server2"):
        if list_key in sanitized and isinstance(sanitized[list_key], list):
            sanitized[list_key] = [
                _sanitize_config_entry_dn(sanitizer, dn)
    ...
    if "differences" in sanitized and isinstance(sanitized["differences"], list):
        ...
        for diff in sanitized["differences"]:
            sd = dict(diff)
            if "dn" in sd:
                sd["dn"] = _sanitize_config_entry_dn(sanitizer, sd["dn"])
