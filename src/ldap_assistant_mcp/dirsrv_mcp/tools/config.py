...
def _replica_role_from_type_flags(
    rtype: Optional[str], rflags: Optional[str]
) -> str:
    """Derive replica role from nsDS5ReplicaType and nsDS5Flags.

    Mirrors lib389's Replica.get_role(): type=3 (read-write) is a supplier,
    type=2 (read-only) is a hub when flags=1 (writes changelog) and a
    consumer when flags=0.
    """
    if rtype == "3":
        return "supplier"
    if rtype == "2":
        return "hub" if rflags == "1" else "consumer"
    return "unknown"
...
def _get_backends_offline(
    ...
    for line in dse._contents:
        ...
        if suffix:
            ...
            rflags = dse.get(replica_dn, "nsds5flags", single=True)
            replica_map[suffix.lower()] = _replica_role_from_type_flags(
                rtype, rflags
            )
