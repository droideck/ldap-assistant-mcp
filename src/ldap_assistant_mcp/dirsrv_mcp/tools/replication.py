...
def _sanitize_replication_result(mcp: "DirSrvMCP", result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "conflicts" in sanitized and isinstance(sanitized["conflicts"], list):
        ...
        for c in sanitized["conflicts"]:
            ...
            if isinstance(nc.get("error"), str):
                nc["error"] = sanitizer.sanitize_text(nc["error"])
            new_conflicts.append(nc)
        sanitized["conflicts"] = new_conflicts
    if "glue_entries" in sanitized and isinstance(sanitized["glue_entries"], list):
        ...
        for g in sanitized["glue_entries"]:
            ...
            if isinstance(ng.get("error"), str):
                ng["error"] = sanitizer.sanitize_text(ng["error"])
            new_glue.append(ng)
        sanitized["glue_entries"] = new_glue
                ...
                try:
                    ...
                    try:
                        ...
                        for agmt in agmts.list():
                            ...
                            if (agmt_details.get("enabled") or "on").lower() == "off":
                                findings.append(
                                    format_finding(
