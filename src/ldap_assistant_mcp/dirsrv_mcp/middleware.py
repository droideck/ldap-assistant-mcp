...
class TimeoutMiddleware(Middleware):
    """Enforce a per-tool-call timeout for cooperative (async) work.
    ...
    Limitation: FastMCP runs *synchronous* tool functions inline on the
    event loop, so this middleware can neither cancel nor even observe a
    timeout while a blocking sync call is running — the timer only fires
    once the event loop regains control.  Blocking I/O must enforce its
    own deadline; LDAP connections do this via the network/operation
    timeouts set in ``connection.connect()`` (``LDAP_CONNECT_TIMEOUT``).
    """
...
class ResponseSizeMiddleware(Middleware):
    ...
    def _guard(self, result: Any) -> Any:
        """Guard both response channels against oversized payloads.

        Text content blocks over the limit are truncated with a notice.
        Oversized ``structured_content`` — the channel modern MCP clients
        actually read — cannot be partially truncated without producing
        invalid JSON, so it is replaced with a small valid error payload.
        """
        result = self._guard_text(result)
        return self._guard_structured(result)

    def _guard_text(self, result: Any) -> Any:
    ...
    def _guard_structured(self, result: Any) -> Any:
        ...
        try:
            serialized_size = len(json.dumps(structured, default=str))
