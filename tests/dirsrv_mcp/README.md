...
Shared pytest fixtures (server config, expected data) live in `conftest.py`. The test files fall into three categories:
...
Following [FastMCP testing guidelines](https://gofastmcp.com/development/tests):
...
# pytest/pytest-asyncio are dev dependencies, not installed by `uv run` alone
uv pip install pytest pytest-asyncio

# Run all dirsrv_mcp tests
uv run pytest tests/dirsrv_mcp/ -v

# Run specific test file
uv run pytest tests/dirsrv_mcp/test_users.py -v

# Run with coverage
uv run pytest tests/dirsrv_mcp/ --cov=ldap_assistant_mcp
```
...
uv run pytest tests/dirsrv_mcp/test_multiserver.py -v
```
